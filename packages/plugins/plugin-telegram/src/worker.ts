...
async function handleUpdate(
  ...
  if (botCommand && config.enableCommands) {
    ...
    if (command === "commands") {
      await handleCommandsCommand(ctx, token, chatId, args, threadId, companyId, baseUrl);
      return;
    }

    const handledCustom = await tryCustomCommand(ctx, token, chatId, command, args, threadId, companyId, baseUrl);
