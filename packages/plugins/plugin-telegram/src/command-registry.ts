...
export async function tryCustomCommand(
  ...
  await executeWorkflow(ctx, token, chatId, cmd, args, messageThreadId, resolvedCompanyId, baseUrl);
  return true;
}
...
async function importCommand(
  ...
  for (const step of definition.steps) {
    ...
    const validTypes = ["fetch_issue", "invoke_agent", "http_request", "send_message", "create_issue", "wait_approval", "set_state", "list_issues"];
...
async function executeWorkflow(
  ...
  for (const step of cmd.steps) {
    try {
      const result = await executeStep(ctx, token, chatId, step, args, results, messageThreadId, companyId, baseUrl);
...
async function executeStep(
  ...
  const interpolate = (template: string): string => {
    ...
    // Replace {{paperclipBaseUrl}} and {{companyId}}
    if (baseUrl) {
      result = result.replace(/\{\{paperclipBaseUrl\}\}/g, baseUrl);
    }
  ...
  switch (step.type) {
    ...
    case "http_request": {
      ...
      // Build headers with auto-injection of auth if needed
      const headers: Record<string, string> = step.headers ? Object.fromEntries(
      ...
      if (contentType.includes("application/json")) {
        try {
          ...
          // Format issue arrays for display
          if (Array.isArray(json)) {
            if (json.length === 0) {
              return "No issues found";
            }
            
            // Check if it's an array of issues (has id, title, status)
            const firstItem = json[0] as any;
            if (firstItem && typeof firstItem === "object" && "title" in firstItem) {
              const formatted = json.map((issue: any) => {
                ...
                return `${emoji} ${id} - ${issue.title} (${status})`;
              }).join("\n");
              
              return formatted || "No issues found";
            }
          }
          
          // Return JSON string for non-issue arrays or objects
    ...
    case "list_issues": {
      ...
      if (step.inboxUnread) {
        filters.touchedByUserId = "me";
        filters.unreadForUserId = "me";
        filters.status = "todo,in_progress,blocked";
      } else if (step.unreadForUser) {
        filters.unreadForUserId = "me";
      }
      if (step.status && !step.inboxUnread) {
        filters.status = interpolate(step.status);
      }
      if (step.projectId) {
        filters.projectId = interpolate(step.projectId);
      }
      if (step.limit) {
        filters.limit = step.limit;
      }
